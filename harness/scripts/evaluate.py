            ...
            for file in files_to_check:
                ...
                for task in task_info.keys():
                    ...
                    if not os.path.isfile(out_file):
                        # Run Locally
                        task_cmd = f"python {work_dir}/main.py --model hf-causal-experimental --model_args pretrained={file} --tasks {task_map} --num_fewshot {num_fewshot} --output_path {out_file}  --device cuda --batch_size 16"
                        cmd = task_cmd
                        # Slurm runner
                        if args.slurm:
